module Routes
  module Generation
    class RunResolver
      ...
      def self.call(company:, scope:, run_id: nil)
        new(company: company, scope: scope, run_id: run_id).call
      end

      def initialize(company:, scope:, run_id:)
        @company = company
        @scope = scope
        @run_id = run_id
      end
      ...
      attr_reader :company, :scope, :run_id
