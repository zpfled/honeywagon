...
RSpec.describe Company::Inventory do
  ...
  describe '#rented_count and #available_count' do
    it 'counts only attached units on blocking orders overlapping the window' do
      blocking_order = create(:order, user: user, status: 'scheduled', start_date: Date.today, end_date: Date.today + 7.days)
      ...
      non_blocking_order = create(:order, user: user, status: 'draft', start_date: Date.today, end_date: Date.today + 7.days)
      ...
      future_order = create(:order, user: user, status: 'scheduled', start_date: Date.today + 30.days, end_date: Date.today + 37.days)
  ...
  describe '#rental_count_for_period' do
    it 'counts only attached units for each billing period' do
      monthly_order = create(:order, user: user, status: 'scheduled')
      ...
      event_order = create(:order, user: user, status: 'scheduled')
